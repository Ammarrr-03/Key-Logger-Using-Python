...
def on_press(key):
    ...
    try:
        # If the key is not already in the pressed set, log it
...
def on_release(key):
    ...
    try:
        ...
        pass  # If the key was not in the set, ignore
...
def send_email_alert(suspicious_activity):
    ...
    try:
        ...
        with smtplib.SMTP('smtp.gmail.com', 587) as server:
            server.starttls()  
            server.login(sender, 'heiq fmnp zert hear')  
...
def detect_keylogger():
    ...
    while is_running:
        ...
        #     print(f'Checking pattern: {pattern} against key sequence: {key_sequence}')  
